...
describe('POST /api/v1/collections', () => {
    it('Should create a collection and send 201', async () => {
        ...
        const res = await request(app).post('/api/v1/collections').send(body).set('Accept', 'application/json').set('Authorization', `Bearer ${token}`)
...
describe('GET /api/v1/collections', () => {
    it('Should get all collections and send 200', async () => {
        const res = await request(app).get('/api/v1/collections').set('Accept', 'application/json')
...
describe('GET /api/v1/collections?id=<collection_id>', () => {
    it('Should get a collection by id', async () => {
        const res = await request(app).get('/api/v1/collections').query({ id: collection_id }).set('Accept', 'application/json')
...
describe('PATCH /api/v1/collections/:id', () => {
  ...
  it('Should edit a collection and send 200', async ()=>{
    const body = {
      name: "new name"
    }     
...
describe('DELETE /api/v1/collections/:id', () => {
    it('Should delete a collection and send 200', async () => {
        const res = await request(app).delete(`/api/v1/collections/${collection_id}`).set('Accept', 'application/json').set('Authorization', `Bearer ${token}`)
    ...
    it('Should fail deleting non-existing collection', async () => {
        const res = await request(app).delete(`/api/v1/collections/${collection_id}`).set('Authorization', `Bearer ${token}`)
